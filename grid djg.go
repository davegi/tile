...
// // Within selects the tiles within a specified bounding box which is specified by
...
// // pagesWithin selects the pages within a specified bounding box which is specified
...
// Neighbors iterates over the Moore neighboring tiles
func (m *Grid[T]) MooreNeighborsDirections(x, y int16, fn func(Point, Tile[T])) {
	directions := []Direction{North, NorthEast, East, SouthEast, South, SouthWest, West, NorthWest}
	p := Point{x, y}
	for _, d := range directions {
		dp := p.Move(d)
		if tile, ok := m.At(dp.X, dp.Y); ok {
			fn(dp, tile)
		}
	}
}
...
// Neighbors iterates over the Moore neighboring tiles
func (m *Grid[T]) MooreNeighborsWithin(x, y int16, fn func(Point, Tile[T])) {
	var dx int16
	var dy int16
	for dx = -1; dx <= +1; dx++ {
		for dy = -1; dy <= +1; dy++ {
			if _, ok := m.At(x+dx, y+dy); ok {
				if _, ok := m.At(x+dx, y+dy); ok {
					nw := At(x+dx, y+dy)
					sw := At(x+dx, y+dy)
					m.Within(nw, sw, func(p Point, t Tile[T]) {
						if p.X == x && p.Y == y {
							return
						}
						fn(p, t)
...
func (m *Grid[T]) VonNeumannNeighbors(x, y int16, fn func(Point, Tile[T])) {
